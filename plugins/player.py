    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        if m.reply_to_message is not None and m.reply_to_message.document:
            if m.reply_to_message.document.file_name != "YouTube_PlayList.json":
                k=await m.reply("Invalid PlayList file given. Use @manaoriginals  or search for a playlist in @manaoriginals to get a playlist file.")
                await delete_messages([m, k])
                return
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/manaoriginals')]]))
    await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        buttons = [
            [
                InlineKeyboardButton('⚡️Join', url='https://t.me/manaoriginals'),
                InlineKeyboardButton('🧩 Join Here', url='https://t.me/manaoriginals'),
            ]
            ]
        await m.reply("<b>You can't use this bot in this group, for that you have to join [SOURCE CODE](https://t.me/manaoriginals) below.</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
